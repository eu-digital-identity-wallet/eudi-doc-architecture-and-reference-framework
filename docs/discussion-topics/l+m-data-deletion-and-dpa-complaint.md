...
### 3.3 Most important currently open questions 

> **General Open Question:**  
> 1) Which **level of technical sophistication** should the *"common protocols and interfaces"* for the two use cases have? 
>    Would it, for example, 
>    * **(a)** be sufficient that the EUDIW simply invokes a mail client with a ``mailto:`` link, 
>    or should 
>    * **(b)** the EUDIW be used for authentication at the wallet-relying party, using a protocol such as e.g. 
>    [OID4VP](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html), or generate a signature one some suitable statement which can be transferred via mail or should there 
>    * **(c)** even be an API provided by the wallet-relying party or the supervisory authority, which allows 
>    to receive requests in manner, which enables automated processing?
