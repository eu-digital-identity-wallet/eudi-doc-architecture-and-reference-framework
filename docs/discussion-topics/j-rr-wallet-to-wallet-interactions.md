...
*It should be noted that the strength of these arguments varies. In particular, the observation and relational-trust considerations in Point 1 do not address the case of a silently compromised Verifier device, and the app-store and development-effort considerations in Point 5 do not address a determined attacker who sideloads a modified or open-source-based wallet.*
...
4. **Linkability risk from certificate reuse.** A WUAC reused across multiple W2W sessions is linkable by its public key and signature. The Subject contains only the Wallet Provider and Wallet Solution names, so individual Users within the same Wallet Solution cannot be distinguished from one another through the certificate fields alone. Tracking a person across encounters in physical proximity is already possible by non-cryptographic means (for example, visual recognition by a single observer present at each encounter). However, the WUAC enables tracking across encounters that share no common physical observer: multiple Verifiers can collaborate after the fact to correlate their sessions with the same Holder by comparing the WUAC public keys they have seen. The risk can be mitigated by issuing a fresh WUAC with a new key pair for each session, at the cost of increased issuance frequency and an online round-trip before each interaction.

*It should be noted that a short-lived WUAC variant could reduce the impact of Point 2 by relying on expiry rather than active revocation, at the cost of amplifying Point 1 through more frequent issuance. This would however not follow [ISO/IEC 18013-5] Annex B CRL Distribution Points requirement.*
...
6. **PID consumption in batch-issued architectures.** The Verifier's PID presentation consumes one PID from the batch on every W2W interaction in which the Verifier acts, even though no PID presentation may have been planned.
...
7. **PID consumption in batch-issued architectures.** As in Approach B, the Verifier's PID presentation in the first (reversed) run consumes one PID from the batch on every W2W interaction, even though no PID presentation may have been planned.
...
5. **Vanilla ISO/IEC 18013-5 interoperability under placements (i) and (iii).** Under placement (i), the WIA-bearing field is additive and vanilla decoders ignore it; vanilla mdocs treat the request as unauthenticated, and a W2W-capable Holder Wallet Unit processing a request from a vanilla reader sees no WIA, which triggers the verification-failure branch of the [Section 3.2](#32-proposed-high-level-requirement-for-discussion) requirements (notify the User, and either not present the requested attributes or give the User the choice). Under placement (iii), vanilla mdoc readers validate `ReaderAuth` exactly as [ISO/IEC 18013-5] specifies, with the `x5chain` certificate chaining to a configured trust anchor; EUDI-aware readers additionally validate the WIA. Placement (ii) does not preserve vanilla interoperability and is retained only for completeness.
...
3. **Linkability risk from WIA reuse.** A WIA reused across multiple W2W sessions is linkable by the Wallet Provider signature and by the `cnf` public key. As in Approach A, tracking a person across encounters in physical proximity is already possible by non-cryptographic means (for example, visual recognition by a single observer present at each encounter). However, the WIA enables tracking across encounters that share no common physical observer: multiple Verifiers can collaborate after the fact to correlate their sessions with the same Holder by comparing the WIAs they have seen. The risk can be mitigated by obtaining a fresh WIA with a new key pair for each session, at the cost of increased issuance frequency and an online round-trip before each interaction.
