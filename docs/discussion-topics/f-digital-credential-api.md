...
In existing implementations there exists a method for Wallet Units to indicate to the operating system the availability of attestations and, optionally, attributes included in the attestation. For example, [this is how this can be done in Android]( https://digitalcredentials.dev/docs/wallets/android/#the-provider-api). According to the provided documentation: 
