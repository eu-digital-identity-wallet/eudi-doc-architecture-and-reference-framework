Version 1.2, updated 22 June 2026
...
This means that the pseudonym registration / pseudonymous authentication and presentations of attributes are separate processes, supported by unrelated specifications (WebAuthn/FIDO2 versus OID4VP/ISO 18013-5). In consequence, there is no mechanism specified for providing cryptographic binding between a pseudonym and (presented) attributes as a proof that they belong to the same User or come from the same Wallet Unit.
The current approach in the ARF is that such binding may be ensured by the RP by its own means. In addition, this approach may impact user experience and increase complexity of the process at the RP side.

Such binding is out of the [WebAuthn] scope. And if a [WebAuthn] credential is stored in the Wallet’s keystore (and not in the WSCD as attestations keys), then such binding is even more problematic. However, this could be possible area of future improvements and work at relevant SDOs.

Therefore the following questions arise:

**QUESTION 1: Is such cryptographic binding desired? If so, should SDO's take initiative to enable cryptographic binding mechanism inherently for WebAuthn/FIDO2-based implementation? 
...
The answer on this question depends on wanted long-term strategy, definition of targeted use cases and suitable types of pseudonyms to support them, and - finally - availability of relevant standards. Such a strategy is yet to be defined. At the same time, as of today, there are no final technical specifications available for Pseudonym Attestations or ZKP-based pseudonyms. Nevertheless, a cryptographic binding mechanism in the long run seems to be necessary to support some of the use cases (e.g. related to use of identity-derived pseudonyms or where the linking back to the User is required). 
...
The answer on this question depends on the long-term strategy, definition of targeted use cases and suitable types of pseudonyms to support them, and - finally - availability of relevant standards. Such a strategy is yet to be defined. At the same time, as of today, there are no final technical specifications available for Pseudonym Attestations or ZKP-based pseudonyms. Nevertheless, support of Scope Rate Limited Pseudonyms in the long run seems necessary.
...
At the same time, linkable pseudonyms could be potentially implemented with other means, e.g. via use of Attested Pseudonyms.
...
The answer on this question depends on the long-term strategy, definition of targeted use cases and suitable types of pseudonyms to support them, and - finally - availability of relevant standards. Such a strategy is yet to be defined. At the same time, as of today, there are no final technical specifications available for Pseudonym Attestations or ZKP-based pseudonyms. Nevertheless, Support of Linkable Pseudonymous Authentication in the long run seems to be necessary, to support some of the use cases (e.g. related to use of identity-derived pseudonyms or where the linking back to the User is required). 
...
The general approach is to enabling various implementation (not only WebAuthn-based) and create more universal HLRs. Therefore this particular aspect is left for implementers do decide, if they choose to implement a WebAuthn-based solution.  
...
It is desirable to define which implementations are relevant and should, or should not, be supported by the EUDI Wallets. 
...
The implementation and characteristics of the pseudonyms strongly depend on the use case. The real life use cases being discussed and tested by Member States currently are for instance recurrent login and account recovery. The general approach is to enable various implementations (not only WebAuthn-based) and create more generic HLRs in this respect. Therefore this particular aspect is left for implementers do decide, if they choose to implement a WebAuthn-based solution.  
...
|-----------|--------------------------------------------------------------|-----------------|
|     PA_01      | A Wallet Unit SHALL enable a User to generate a Pseudonym and register it at a Relying Party.**The Pseudonym may be user-chosen or identity-derived; the Wallet Unit SHALL support at least one of those types of a Pseudonym.** ~~For an attested pseudonym, pseudonym generation takes place by requesting the issuance of a pseudonym attestation. Pseudonym registration takes place by presenting the attestation._~~ | Keep with the proposed change |
|     **PA_01a**      | **When a Pseudonym is user-chosen, it SHALL be generated and fully controlled by the User or User's device. The Pseudonym SHALL not be cryptographically linked to User's PID, Attestation or another User-related identifier, that is not under full control of the User. _Note: A WebAuthn/FIDO2 authenticator is an example of a user-chosen Pseudonym._** | New requirement  |
|     **PA_01b**  | **When a Pseudonym is identity-derived, it SHALL be generated from User's PID, Attestation or another User-related identifier, that is not under full control of the User. The Pseudonym SHALL ensure cryptographic linking to the User's PID, Attestation or identity. _Note: A Pseudonym Attestation is an example of an identity-derived Pseudonym._** | New requirement |
...
|     **PA_02a**    |  **A Wallet Unit SHALL ensure unlinkability of Pseudonyms. The charachteristics of unlikability property SHALL be aligned with the use case and it MAY mean one or more from the following: (a) unlinkability to the User's identity, PID or Attestation, (b) unlinkability to the PID or Attestation Provider, (c) unlinkability to the Wallet Solution Provider, (d) unlinkability between authentication transaction of the same User, (e) unlinkability between Relying Parties, (f) linkability between a group of Relying Parties, (g) linkability with identity (under certain conditions, eg. for law enforcement purposes). The Wallet Unit SHOULD enable Relying Parties to request authentication with a Pseudonym meeting criteria (for type and unlinkability characteristics) for a given use case.**  |  New requirement |
|     **PA_02b**    |  **For user-chosen Pseudonyms, a Wallet Unit SHALL enable at least unlinkability with User's identity and PID and Attestations (characteristics (a) from PA_02a), unlinkability between authentication transaction of the same User (characteristic (d) from PA_02a), unlinkability between Relying Parties (characteristic (e) from PA_02a). _Note: A WebAuthn/FIDO2 authenticator is an example of a Pseudonym meeting this requirement._**  |  Keep with the proposed change |
|     **PA_02c**    |  **For identity-derived Pseudonyms, a Wallet Unit SHOULD enable linkability with identity under certain conditions (characteristic (g) from PA_02a).** | New requirement |
|     **PA_02d**    | **A Wallet Provider SHALL ensure that Wallet Unit's User and Relying Parties are clearly informed about the types and characteristics of supported Pseudonyms by the Wallet Unit.** | New requirement | 
...
|     **PA_04a**      | **A Wallet Unit SHALL enable the User to use a single Pseudonym at a given Relying Party. _Note: A WebAuthn/FIDO2 authenticator is an example of a Pseudonym meeting this requirement._**  |   New requirement  |
|     **PA_04b**      | **A Wallet Unit SHOULD enable the User to use a single Pseudonym with multiple Relying Parties.**  |   New requirement  |
...
|     PA_10      | A Relying Party SHALL be able to verify that a User is registering a Pseudonym using a non-revoked Wallet Unit.   | Keep as-is |
|     PA_11      | A Relying Party SHALL be able to verify that a User is authenticating with a Pseudonym using a non-revoked Wallet Unit.   | Keep as-is |
|     PA_12      | If Wallet Unit is used to register a Pseudonym at a Relying Party in combination with a PID or attestation being presented to the same Relying Party, then this Relying Party SHALL be able to verify that the same User performed both actions. |  Keep as-is |
|     PA_13    | ~~The Relying Party SHALL be able to validate that the pseudonym presented to them belongs to the User presenting it.~~ **A Wallet Unit SHALL enable a Relying Party to verify that a Pseudonym belongs to the User using it**.  | Keep with the proposed change |
|     PA_14      | A Wallet Unit SHALL ~~store the information necessary for authenticating with~~ **store Pseudonyms locally, encrypted, as well as enable binding** a Pseudonym to ~~in~~ **WSCD/WSCA or** a keystore. | Keep with the proposed change |
...
|     PA_24      | A protocol enabling scope rate-limited pseudonyms SHALL enable a Wallet Unit to allow a User to generate a scope rate-limited pseudonym, register this by a Relying Party, and prove that this is within the rate and scope restrictions determined by the Relying Party.  | Keep as-is  |
|     PA_25      | A protocol enabling scope rate-limited pseudonyms SHALL allow a Relying Party, when a User presents a scope rate-limited pseudonym, to verify that the rate is not exceeded for this User.  | Keep as-is  |
|     PA_26      | A protocol enabling scope rate-limited pseudonyms SHALL allow a Relying Party to choose the scope and rate when requesting a scope rate-limited pseudonym from a User. | Keep as-is  |
|     PA_27      | A protocol enabling scope rate-limited pseudonyms SHALL NOT allow any entity or collusion of entities not including the User, to link scope rate-limited pseudonyms of the same User when used across several different Relying Parties. This SHALL hold even if the scope and rate are identical across the different Relying Parties and both for registration and authentication of the scope rate-limited pseudonym. | Keep as-is |
|     PA_28      | A protocol enabling scope rate-limited pseudonyms SHALL ensure that if the rate is larger than 1, a User's different pseudonyms SHALL be unlinkable for the same scope. This SHALL hold against any entity or collusion of entities, not including the User. Further, such protocol SHALL ensure that during registration or authentication with such pseudonym, it SHALL NOT be possible for the Relying Party to deduce any information about how many pseudonyms the User has already registered (except that it does not exceed the predetermined rate). | Keep as-is  |
|     PA_29      | A protocol enabling scope rate-limited pseudonyms SHALL ensure that no entity or collusion of entities, not including a User, is able to authenticate or register with a scope rate-limited pseudonym of this User. | Keep as-is |
|     PA_30      | ~~A Wallet Unit SHALL store cryptographic material necessary for authenticating as a scope rate-limited pseudonyms in either a WSCA/WSCD or in a keystore.~~ | Remove (as covered by PA_14) |
|     PA_31      | A User's scope rate limited pseudonyms for a particular scope and rate SHALL be persistent over time even if they start using another Wallet Unit. | Keep as-is  |
