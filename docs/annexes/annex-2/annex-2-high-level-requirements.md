...
| WUA_07  | A Wallet Unit SHALL present a WUA only as part of either the issuance of a PID or attestation, or in conjunction with the presentation of a PID or another attestation to a Relying Party. *Note: the WUA presented during issuance has different information content and may have a different format than the WUA presented in conjunction with a PID or an attestation.*  |
...
| **Index** | **Requirement specification** |
|-----------|-----------------|
| EDP_01 | A Wallet Unit SHALL enable an Attestation Provider to optionally express embedded disclosure policies for a QEAA, PuB-EAA, or non-qualified EAA. *Note: The [European Digital Identity Regulation] does not contain a requirement for PIDs to be able to contain an embedded disclosure policy.* |
