...
| WUA_07  | A Wallet Unit SHALL present a WUA only as part of either the issuance of a PID or attestation, or in conjunction with the presentation of a PID or an attestation to a Relying Party. *Note: the WUA presented during issuance has different information content and may have a different format than the WUA presented in conjunction with a PID or attestation.*  |
