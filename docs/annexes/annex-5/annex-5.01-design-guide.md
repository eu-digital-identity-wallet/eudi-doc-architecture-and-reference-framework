...
_This is a working document that holds no legal value and does not reflect any common agreement or position of the co-legislators. It presents a state-of-play of ongoing work of the eIDAS Expert Group. This document is being continuously updated and should not be considered final._
...
Twelve design criteria have been selected which we go over in details in 'Section 2'. The first 10 are the usability heuristics from the [Nielsen Norman group](https://www.nngroup.com/articles/ten-usability-heuristics/). They are called "heuristics"  because they are broad rules of thumb and not specific usability guidelines. These are used to evaluate a User Interface, so it is good to have them as guiding principles during the design phase as well. These 10 principles are:
...
![Document Management example showing success and failure states](images/5.01/Figure_1_Document_Management_example.png)
...
![Interactive Elements example showing button states](images/5.01/Figure_2_Interactive_Elements_example.png)
...
![Document Presentation Example showing realistic document representations](images/5.01/Figure_3_Document_Presentation_Example.png)
...
![Labels example showing user-friendly terminology](images/5.01/Figure_4_Labels_example.png)
...
![Icons Example showing clear vs ambiguous icons](images/5.01/Figure_5_Icons_Example.png)
...
![Undo and Redo Example showing action reversibility](images/5.01/Figure_6_Undo_and_Redo_Example.png)
...
![Confirmation Dialogs Example showing user verification before action](images/5.01/Figure_7_Confirmation_Dialogs_Example.png)
_Figure 7: Confirmation Dialogs Example_
...
![Flexible Inputs Example showing different input format acceptance](images/5.01/Figure_8_Flexible_Inputs_Example.png)
...
![Error Messages Examples showing clear problem indications and solutions](images/5.01/Figure_9_Error_Messages_Examples.png)
...
![Layout diagram showing thumb reachable areas on mobile screen](images/5.01/Figure_10_Layout.png)
_Figure 10: Layout_
...
A target size is the area that can be activated in order to interact with an element. Individuals with dexterity challenges may find it more challenging to utilize a website if the target size is smaller. In this section, we'll examine methods ([Apple's HIG](https://developer.apple.com/design/human-interface-guidelines/layout) and [Google's Material](https://m3.material.io/foundations/layout/understanding-layout/overview)) for generating target sizes that are user-friendly, uniform, and properly spaced. A person's ability to interact with smaller controls may be impacted by a disability or a combination of disabilities that affect their motor movements and dexterity, as well as by the act of using a website while on the move, such as while walking or commuting.

![Target Sizes diagram showing recommended touch target dimensions](images/5.01/Figure_11_Target_Sizes.png)
...
- **Informative** (Motion design informs users by highlighting relationships between elements,
    action availability, and action outcomes.)
...
![Example showing "Document added" text](images/5.01/Document_added.png)

Write in the present tense.

![Example showing "Document has been added" text](images/5.01/Document_has_been_added.png)
...
```
![Example showing "To add a document, click +" text](images/5.01/To_add_a_document.png)


Start a statement with the objective ("to add a document") and end it with the user action
("click + ").

```
Click + to add a document
```
![Example showing "Click + to add a document" text](images/5.01/Click_to_add_a_document.png)
...
```
![Example showing "Change your preferences in My Account" text](images/5.01/Change_your_preferences_in_My_Account.png)
...
```
![Example showing "Change your preferences in Account" text](images/5.01/Change_your_preferences_in_Account.png)
...
- _Guidelines around 'user consent'_ in data sharing scenarios (e.g. requesting user consent,
    enforcing trust)
