...
![Login screen with EUDI Wallet login option on desktop browser](images/5.02/Login_with_EUDI_wallet2.png)
...
![Share information screen showing QR and NFC sharing options](images/5.02/Share_information.png)
...
A set of 'authentication means' applicable for the EUDI Wallet are being analysed in this
...
![Multiple authentication method screens showing PIN, pattern, biometrics, password and OTP options](images/5.02/Identification_methods.png)
...
![Radar chart comparing different authentication methods across multiple criteria](images/5.02/Authentication_means.png)

> **_NOTE:_** _Ratings have been based on a desk study and not actual first-hand testing._
...
| Method | Pros | Cons |
|--------|------|------|
| **PIN** | • Short and easy authentication method<br>• Flexibility in PIN requirements | • Slower unlocking compared to other authentication methods<br>• Requires users to memorize numbers<br>• Recovery can be hard if you forget the PIN<br>• Often predictable |
| **Pattern** | • Simple and intuitive to use | • Many people choose simple, predictable patterns<br>• Input method is visible to those around you<br>• Belongs to a third party |
| **Password** | • More secure than a PIN<br>• Flexibility in password requirements | • Easy to guess<br>• Slower unlocking<br>• Password recovery can be as hard as a PIN recovery |
| **Biometrics (fingerprint)** | • Fast and convenient authentication method | • Fingerprints can be replicated<br>• Fingerprint distortion can cause failures<br>• Belongs to a third party |
| **Biometrics (face scan)** | • Fast unlocking method<br>• It doesn't require memorizing codes and passwords | • Light effects and facial changes can cause failures<br>• Screen orientation and distance from the camera can impact readability<br>• The scanner can be fooled by user's photos or sometimes familial similarities<br>• Provided by a third party |
| **One Time Password (OTP)** | • Alleviates the burden associated with memorizing passwords<br>• Usually utilized as 2FA on top of PIN/Passwords but may also <br> &nbsp;  be used as an alternative to passwords (applicable after first  <br> &nbsp; registration to a service) <br>• Offers a sense of advanced safety for the user | • Associated with higher 'interaction cost' (i.e. users are requested to type a code)<br>• May raise confusion if OTP is not received on time – multiple attempts to receive an OTP<br>• May require clear and concise OTP text (e.g. SMS or email) |
...
![Data sharing screens showing mandatory and optional information requests from Relying Party](images/5.02/RP_requests_info.png)
...
- **5.1 Erroneous user credentials**
  ...
  ![Login error screen showing incorrect password message](images/5.02/Erroneous_user_credentials.png)
...
- **5.2 Multiple failed attempts to login or present information**
  ...
  ![Login screen showing too many attempts error with timer](images/5.02/Erroneous_user_credentials2.png)
...
- **5.3 The document is considered invalid (expired/revoked)**
  ...
  ![Error message showing additional documents required](images/5.02/Error.png)
  ...
  ![Warning notification about document expiration with renewal option](images/5.02/Warning.png)
...
- **5.4 The Relying party is not considered trusted. Is not verified or could not be verified (Maybe address safety)**
  ...
  ![Alert warning about untrusted relying party with security options](images/5.02/Alert.png)
  
  _The user gets a message indicating that they must not share information with that party. The options are to report it, to close the app, or to search for information about security:_
...
- **5.5 The user fails to present requested document**
  ...
  ![Error message showing missing ID card document with add document option](images/5.02/Error2.png)
  
  _The error message then suggests adding the document from the available documents list._